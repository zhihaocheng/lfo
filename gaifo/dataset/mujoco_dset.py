...
class Mujoco_Dset(object):
    def __init__(self, expert_path, train_fraction=0.7, traj_limitation=-1, randomize=True):
        ...
        if len(obs.shape) > 2:
            obs_tmp = np.reshape(obs, [-1, np.prod(obs.shape[2:])])
        else:
            obs_tmp = np.vstack(obs)

        self.obs = obs_tmp[0:-1]
        self.obsn = obs_tmp[1:]
        
        self.rets = traj_data['ep_rets'][:traj_limitation]
        self.avg_ret = sum(self.rets)/len(self.rets)
        self.std_ret = np.std(np.array(self.rets))
        assert len(self.obs) == len(self.obsn)
        self.num_traj = min(traj_limitation, len(traj_data['obs']))
        self.num_transition = len(self.obs)
        self.randomize = randomize
        self.dset = Dset(self.obs, self.obsn, self.randomize)
        # for behavior cloning
        self.train_set = Dset(self.obs[:int(self.num_transition*train_fraction), :],
                              self.obsn[:int(self.num_transition*train_fraction), :],
                              self.randomize)
